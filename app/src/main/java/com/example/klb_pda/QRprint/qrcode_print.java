...
// Clean up the objects created by EMDK manager
    ...
    private void creatlist(int num) {
        try {
            db.close();
            db.open();
            final int count = Integer.parseInt(edt3.getText().toString());//總數量
            int qty = Math.round(count / num);//拆標後平均數量
            int count2 = count;//計算餘數
            //取得單頭資料
    ...
    private String recal_qty() {
        String res = "OK";
        Integer val_edt3 = Integer.parseInt(edt3.getText().toString());
        Integer val_edt4 = Integer.parseInt(edt4.getText().toString());
        Integer sumY = 0, sumN = 0, countY = 0, countN = 0;
        Integer tt_slpb = 0, slpb = 0;

        try {
            Cursor g_cursor = db.sum_qrb03("Y");
            g_cursor.moveToFirst();
            sumY = Integer.parseInt(g_cursor.getString(g_cursor.getColumnIndex("SUMQRB03"))); //條碼總數
            if (sumY > val_edt3) {
                res = "FAIL";
            }
            ...
            g_cursor.moveToFirst();
            sumN = Integer.parseInt(g_cursor.getString(g_cursor.getColumnIndex("SUMQRB03"))); //條碼總數
            ...
            slpb = Math.round(tt_slpb / countN);
    ...
    private void printSample2(final Cursor cursor1) {
        try {
            if (mSampleDialog == null) {
                ...
                @SuppressLint("Range") Integer xqrb03 = cursor1.getInt(cursor1.getColumnIndex("qrb03"));
