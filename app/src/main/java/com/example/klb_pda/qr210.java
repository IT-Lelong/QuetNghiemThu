...
// Clean up the objects created by EMDK manager
    ...
    private void updatedetail(String datastr) {
        try {
            if (datastr.startsWith("new")) {
                ...
                //料號
                String qr01 = datastr.substring(4, index2);
                //批號
                String qr02 = datastr.substring(index2 + 1, index3);
                //數量
                ...
                Thread api = new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        //取得批號
                        String qr02 = getdatecode("http://172.16.40.20/" + g_server + "/QR220/get_datecode.php?code=" + datastr + "&kind=" + 2);
                        //取得數量
                        Integer qr03 = Integer.valueOf(datastr.substring(index3 + 1));
                        scan(datastr, qr01, qr02, qr03);
                    }
                ...
                Thread api = new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        //取得批號
                        String qr02 = getdatecode("http://172.16.40.20/" + g_server + "/PDA_QR210/get_datecode.php?code=" + datastr + "&kind=" + 1);
                        //取得數量
        ...
        try {
            if (cursor != null && cursor.getCount() >= 0) {
                SimpleCursorAdapter adapter = new SimpleCursorAdapter(this, R.layout.activity_qr210_view01, cursor,
                        new String[]{"rownum", "qr210_01", "qr210_02", "qr210_04", "qr210_05"},
        ...
        try {
            if (cursor != null && cursor.getCount() >= 0) {
                /*SimpleCursorAdapter adapter=new SimpleCursorAdapter(qr210.this,R.layout.activity_qr210_dialog01_view,cursor,
                        new String[]{"rownum","qr210b_02","qr210b_03"},
    ...
    private AdapterView.OnItemClickListener lsit01listener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
            ...
            //dialoglist01.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE);
            ...
            dialoglist01.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    if (simpleCheckedTextView.isChecked()) {
                        db.updCheckBox(g_dialogitem01, qr210_01, g_dialogitem03, false);
                        Cursor cursor = db.getdialogdetail(qr210_01);
                        UpdateAdapterdialog(cursor);
                    } else {
                        db.updCheckBox(g_dialogitem01, qr210_01, g_dialogitem03, true);
                        Cursor cursor = db.getdialogdetail(qr210_01);
                        UpdateAdapterdialog(cursor);
                    }
                }
    ...
    private View.OnClickListener btnuploadlistener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            if (head1.getText().length() > 0) {
                                    ...
                                    qr210.this.runOnUiThread(new Runnable() {
                                        ...
                                        public void run() {
                                            ...
                                            builder.setPositiveButton(getString(R.string.main_ok), new DialogInterface.OnClickListener() {
                                                @Override
                                                public void onClick(DialogInterface dialogInterface, int i) {
                                                    qr210_mail("http://172.16.40.20/" + g_server + "/PDA_QR210/mail.php?ID=" + ID);
                                                }
                ...
                Toast.makeText(qr210.this, R.string.qr210_msg16, Toast.LENGTH_SHORT).show();
            }
        }
    };
    ...
    //發送mail
    private void qr210_mail(String apiurl) {
